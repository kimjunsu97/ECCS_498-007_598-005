...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    for i in range(len(x_train_folds)):
      train_data_x = torch.cat((x_train_folds[:i] + x_train_folds[i + 1:]), dim=0)
      train_data_y = torch.cat((y_train_folds[:i] + y_train_folds[i + 1:]))

      classifier = KnnClassifier(train_data_x, train_data_y)
      valid_data_x = x_train_folds[i]
      valid_data_y = y_train_folds[i]

      k_to_accuracies[k].append(classifier.check_accuracy(valid_data_x, valid_data_y, k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
